...
static uint16_t dmxMax = 511; /* Default to sending the first 16 channels */
...
void dmxWrite(int* channelAry) {

  int array_size = 511;

  for(int i = 0; i < array_size; i++)
  {
    dmxBuffer[i] = channelAry[i];
  }
  dmxBegin();
...
void DmxMasterClass::write(int* channelAry) {
  dmxWrite(channelAry);
}
